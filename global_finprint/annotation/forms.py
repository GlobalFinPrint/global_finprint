...
class SingleSelectizeWidget(forms.Select):
    template = '<div class="sub-control"><select class="selectize form-control"{}>'
...
class MultiRowTextInput(forms.Widget):
    ...
    def render(self, name, value, attrs=None):
        html = ''
        for val in value:
            html += self.template.format(name, '' if val is None else val)
...
class MultiRowRadioSelect(forms.Widget):
    ...
    def render(self, name, value, attrs=None):
        html = ''
        for _ in value:
            html += self.template.format(name)
        return mark_safe(html)
...
class VideoForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['primary'].choices = [(0, '')]
        self.fields['primary'].initial = 0
        self.fields['remove_row'].initial = [0]

        if video is not None:
            video_files = video.files.order_by('rank')
            row_nums = list(range(max(video_files.count(), 1)))
            ...
            self.fields['primary'].choices = row_nums
            self.fields['primary'].initial = list(f.primary for f in video_files)
            self.fields['remove_row'].initial = row_nums
